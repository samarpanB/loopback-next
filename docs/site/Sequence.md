...
In LoopBack, we use `try`/`catch` flow to receive errors from individual
sequence actions, and call Sequence action `reject` to handle them, as described
in [Handling errors](Sequence.md#handling-errors).
